...
def _extract_repeated_phrases(comments: list[YouTubeCommentSample], limit: int = 8) -> list[str]:
    bigram_counter: Counter[str] = Counter()

    for comment in comments:
        normalized = _normalize_text(comment.text_display)
        words = [word for word in normalized.split() if word and word not in STOPWORDS and len(word) > 2]
...
def summarize_evidence(
    ...
    praise_count = sum(
        ...
        if _contains_any(text, ["love", "great", "amazing", "helpful", "useful", "best", "excellent"])
    )

    request_count = sum(
        1
        for text in normalized_comments
        if _contains_any(text, ["please", "can you", "could you", "would love", "want", "need", "make a video"])
    )

    pain_count = sum(
        1
        for text in normalized_comments
        if _contains_any(text, ["confused", "struggle", "hard", "difficult", "problem", "issue", "dont understand", "do not understand"])
    )
    ...
    if praise_count > 0:
        praise_themes.append("Viewers respond positively to the current content style or usefulness.")
    if request_count > 0:
        request_themes.append("Commenters are asking for more content, follow-ups, or direct answers.")
    if pain_count > 0:
        pain_points.append("Some commenters are expressing confusion, difficulty, or unresolved problems.")
...
def build_assumptions_from_evidence(
    ...
    if not audience_interests:
        audience_interests = [
            ...
            f"{channel_niche} practical tips",
            f"{channel_niche} explainers",
        ]

    likely_topic_patterns = summary.top_video_titles[:3]
    if not likely_topic_patterns:
        likely_topic_patterns = [
            f"{channel_niche} explainers",
            f"{channel_niche} opinion or breakdown content",
        ]

    intent_parts: list[str] = []

    if summary.request_themes:
        intent_parts.append("The audience appears to want more direct answers and follow-up content.")
    if summary.pain_points:
        intent_parts.append("There are signs that viewers want clarity on difficult or confusing topics.")
    if not intent_parts:
        intent_parts.append(
    ...
    confidence_parts = [
        "These assumptions were drafted from fetched YouTube titles and sampled top-level comments.",
